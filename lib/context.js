import {cloneDeepWith, sortBy} from 'lodash-es';
...
export default class TestaContext {
	...
	* @param {Boolean} [options.symmetric=false] Sort keys before saving for a side-by-side JSON diff
	*
	* @returns {TestaContext} This chainable instance
	*/
	dump(...msg) {
		...
		} else if (typeof msg.at(-1) == 'object' && 'symetric' in msg.at(-1)) { // Last argument looks like its a settings object
		...
		// Sort keys if we're working symetricly
		if (settings.symetric) {
			dumpFileOutput = cloneDeepWith(dumpFileOutput, node =>
				typeof node == 'object'
					? Object.fromEntries(
						sortBy(Object.entries(node), i => i[0]) // Sort output object [key, val] by key
					)
					: undefined // Let cloneDeepWith() do its default clone behaviour
			);
		}
