...
export class TestaBase {
	...
	*/
	exec(tests, options) {
		...
		return Promise.resolve()
			...
			.then(runnables => ({
				runnables,
				graph: runnables // Compute dependency graph
					.filter(test => test._depends && test._depends.length > 0)
					.flatMap(test => test._depends.map(depend =>
						[depend, test._id]
					))
			}))
			.then(({runnables, graph}) => {
				if (this.debug) console.log('Computed test dependency graph', graph);
				return {runnables, graph};
			})
			.then(({runnables, graph}) => pGraph(
	...
	*/
	execAll(options) {
		...
		// Do we have any `series()` marked tessts? - If so queue up all tests before them as dependencies
		tests
			.forEach((test, testIndex) => { // NOTE: We can't use the list of tests post-filter as we would loose the run order
				if (!test._series) return; // Not in series mode
				let preDeps = testIndex >= 0 ? tests.slice(0, testIndex).map(t => t._id) : [];
				let postDeps = testIndex >= 0 && testIndex < tests.length - 1 ? tests.slice(testIndex + 1).map(t => t._id) : [];
				if (this.debug) console.log('Applying series() dependencies to', test.toString(), {
					offset: testIndex,
					id: test._id,
					pre: preDeps,
					post: postDeps,
