...
* This also outputs a Chai diff if one is provided in the error payload
*
...
export function cleanError(err, options) {
	...
	// Show the Chai diff report if we have one
	if (err.expected && err.actual) {
		console.log(diff(err.expected, err.actual, {
			expand: false, // Dont show everything - only changes
			contextLines: 2, // Lines of context around changes
...
/**
* Create a shallow copy of the input object but with all object keys sorted alphabetically
*
* @param {*} val The input value
* @returns {*} The input value but with all deep object keys sorted
*/
